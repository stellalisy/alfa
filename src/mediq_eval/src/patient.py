...
class Patient:
    def __init__(self, args, sample):
        ...
        if isinstance(sample['context'], list) and len(sample['context']) > 0:
            if args.healthQ_use_original_post_as_initial_info: self.initial_info = sample['original_post']
            elif 'initial_info' in sample: self.initial_info = sample['initial_info']
