...
class ModelCache:
    ...
    def get_next_available_device(self):
        ...
        available_devices = available_devices[1:] + [available_devices[0]]
        #temporary fix, need to figure out how to specify more than one gpu for device_map lol
        return device
    ...
    def load_model_and_tokenizer(self):
        if self.use_api == "openai":
            ...
            try:
                ...
                device = self.get_next_available_device()
                enable_prefix_caching = self.args.get("enable_prefix_caching", False)
                self.model = LLM(model=self.model_name, enable_prefix_caching=enable_prefix_caching, device=device)
                from transformers import AutoTokenizer
                try:
                    self.tokenizer = AutoTokenizer.from_pretrained(self.model_name, device_map=device, trust_remote_code=True)
                except:
                    print(f"Failed to load tokenizer from {self.model_name}, try to load from base model")
                    if "8b" in self.model_name: tokenizer_path = "meta-llama/Llama-3.1-8B-Instruct"
                    elif "3b" in self.model_name: tokenizer_path = "meta-llama/Llama-3.2-3B-Instruct"
                    else: tokenizer_path = "meta-llama/Llama-3.1-70B-Instruct"
                    self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, device_map=device, trust_remote_code=True)
        ...
        if not self.use_vllm and self.use_api != "openai":
            ...
            try:
                ...
                self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, device_map=device, trust_remote_code=True)
            self.model = AutoModelForCausalLM.from_pretrained(self.model_name, device_map=device)
