...
def question_generation_healthQ(patient_state, inquiry, options_dict, messages, no_system_prompt, **kwargs):
    patient_info = patient_state["initial_info"] + "\nPatinet inquiry: " + inquiry + "\nOptions: " + ', '.join([f"{k}: {v}" for k, v in options_dict.items()])

    messages = [
        # {"role": "system", "content": prompts.healthQ_prompts["meditron_system_msg"]},
        # {"role": "system", "content": prompts.healthQ_prompts["system"]},
        {"role": "system", "content": "Ask an atomic question to the patient to acquire more information to help you answer the patient inquiry. An atomic question is a q uestion that can be answered with a single piece of information. Respond with the atomic question only and nothing else."},
    ...
    response_text, atomic_question, num_tokens = expert_basics.expert_response_question(messages, **kwargs)
    log_info(f"[HEALTH_Q ATOMIC QUESTION PROMPT]: {messages}")
    log_info(f"[HEALTH_Q ATOMIC QUESTION RESPONSE]: {atomic_question}\n")
    messages.append({"role": "assistant", "content": atomic_question})

    log_info(f"[HEALTH_Q ATOMIC QUESTION RETURN]: {atomic_question}, usage: {num_tokens}\n")
    return {
        "atomic_question": atomic_question,
        "messages": messages,
        "usage": num_tokens,
