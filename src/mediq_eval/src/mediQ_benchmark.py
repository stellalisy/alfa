...
def main():
    ...
    print(f"Accuracy: {sum(correct_history)} / {len(correct_history)} = {sum(correct_history) / len(correct_history)}")
    print(f"Timeout Rate: {sum(timeout_history)} / {len(timeout_history)} = {sum(timeout_history) / len(timeout_history)}")
    print(f"Avg. Turns: {sum(turn_lengths) / len(turn_lengths)}")
