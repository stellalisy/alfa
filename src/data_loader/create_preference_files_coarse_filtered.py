# huggingface-cli upload stellalisy/healthQ_preference [local_path] [path_in_repo] --repo-type dataset
...
def build_counterfactual_pairs(data, good_ids, features=["overall"]):
    """
        ...
        for feature in features:
            ...
            for a, r in [(more_key, less_key), (more_key, "original"), ("original", less_key)]: 
                ...
                if good_ids != None and pair_id not in good_ids:
                    continue
                pair_id.replace("_overall_", "_coarse_")
...
data_dir = ""
good_ids_file = f"{data_dir}verification/train_good_ids_overall.jsonl"
input_filepath = f"{data_dir}train_counterfactuals.jsonl"
output_filepath = f"preference/coarse_filtered/train.jsonl"
split_save_data(data_file=input_filepath, output_file=output_filepath, good_ids_file=good_ids_file, write=False)

good_ids_file = f"verification/eval_good_ids_overall.jsonl"
input_filepath = f"eval_counterfactuals.jsonl"
output_filepath = f"preference/coarse_filtered/validation.jsonl"
split_save_data(data_file=input_filepath, output_file=output_filepath, good_ids_file=good_ids_file, write=False)