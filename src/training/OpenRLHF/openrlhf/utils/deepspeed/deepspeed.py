...
class DeepspeedStrategy(ABC):
    ...
    def save_model(self, model: nn.Module, tokenizer, output_dir, **kwargs) -> None:
        ...
        if self.is_rank_0():
            ...
            assert output_state_dict_keys.issubset(
                state_dict_keys
            ), f"mismatch keys len(output_state_dict_keys)={len(output_state_dict_keys)}, len(state_dict_keys)={len(state_dict_keys)}\n{state_dict_keys.symmetric_difference(output_state_dict_keys)}"
