...
def train(args):
    ...
    #     model.model.push_to_hub("stellalisy/TBD")
    ...
    for data in eval_dataloader:
        if not args.packing_samples:
            ...
            prompt_ids = [chosen_ids[i, : prompt_id_lens[i]].unsqueeze(0) for i in range(len(prompt_id_lens))]
        else:
            ...
            prompt_ids = [packed_input_ids[0].split(packed_seq_lens)[i][:prompt_id_lens[i]].unsqueeze(0) for i in range(len(prompt_id_lens))]
