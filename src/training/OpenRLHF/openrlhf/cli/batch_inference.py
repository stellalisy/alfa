...
def batch_generate_vllm(args):
    ...
    try:
        ...
        print(f"Failed to load tokenizer from {args.pretrain}, try to load from base model")
        if "8b" in args.pretrain: tokenizer_path = "meta-llama/Llama-3.1-8B-Instruct"
        elif "3b" in args.pretrain: tokenizer_path = "meta-llama/Llama-3.2-3B-Instruct"
    ...
    prompts_dataset = PromptDataset(prompts_data, tokenizer, dummy_strategy, input_template=args.input_template, healthq_question_system_prompt=args.healthq_question_system_prompt)
...
def batch_generate(args):
    ...
    for prompts, qids in pbar:
        ...
        for best_of_n_i in range(N):
            ...
            for prompt, qid, input_ids, output_ids in zip(prompts, qids, inputs["input_ids"], outputs):
                output_text = tokenizer.decode(output_ids[len(input_ids):], skip_special_tokens=True)
                output_dataset.append({"id": qid,
                                       "generator": args.generator_name,
                                       "best_of_n": best_of_n_i,
                                       "input": prompt, 
                                       "output": output_text})
        dist.barrier()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--generator_name", type=str, default=None, help="model identifier for saving outputs to file")
    parser.add_argument(
    ...
    parser.add_argument("--healthq_question_system_prompt", action="store_true", default=False)
    ...
    if not args.generator_name: args.generator_name = args.pretrain